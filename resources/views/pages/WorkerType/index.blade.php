...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Worker Type</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('worker-type.index') }}">Worker Type</a></li>
                        <li class="breadcrumb-item active">Worker Type</li>
                    </ol>
                </div>
            </div>
            <div class="row mb-2">
                <div class="col-sm-6">
                    <a href="{{ route('worker-type.create') }}" class="btn btn-primary">Create Worker Type</a>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <tbody>
                            @foreach ($worker_types as $worker_type)
                                <tr>
                                    ...
                                    <td>{{ $worker_type->codeDirectory->name }}</td>
                                    <td>{{ $worker_type->table_name_label }}</td>
                                    <td>{{ $worker_type->created_at }}</td>
                                    <td>{{ $worker_type->updated_at }}</td>
                                    <td>
                                        <a href="{{ route('worker-type.show', $worker_type->id) }}"
                                            title="View">
                                            <i class="nav-icon fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('worker-type.edit', $worker_type->id) }}"
                                            title="Edit">
                                            <i class="nav-icon fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('worker-type.destroy', $worker_type->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
