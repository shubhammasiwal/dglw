...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Address Type</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('address-type.index') }}">Address Type</a></li>
                        <li class="breadcrumb-item active">Address Type</li>
                    </ol>
                </div>
            </div>
            <div class="row mb-2">
                <div class="col-sm-6">
                    <a href="{{ route('address-type.create') }}" class="btn btn-primary">Create Address Type</a>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <tbody>
                            @foreach ($address_types as $address_type)
                                <tr>
                                    ...
                                    <td>{{ $address_type->codeDirectory->name }}</td>
                                    <td>{{ $address_type->table_name_label }}</td>
                                    <td>{{ $address_type->created_at }}</td>
                                    <td>{{ $address_type->updated_at }}</td>
                                    <td>
                                        <a href="{{ route('address-type.show', $address_type->id) }}"
                                            title="View">
                                            <i class="nav-icon fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('address-type.edit', $address_type->id) }}"
                                            title="Edit">
                                            <i class="nav-icon fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('address-type.destroy', $address_type->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
